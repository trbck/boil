...
- **[`superpowers`](https://github.com/obra/superpowers)** — mandatory, not optional (hard rule 9): every implementer attempt runs under `test-driven-development`, adds `systematic-debugging` once a counterexample exists, and ends with `verification-before-completion`; the dispatch packet lists them and a return without a `Skills invoked:` line is not a done claim. `brainstorming` shapes the goal in Phase 0; `dispatching-parallel-agents` / `subagent-driven-development` drive T2+ dispatch.
