...
2. **Act with judgment** — proceed on safe, reversible, in-scope work; ask only when the missing decision is material, authority is absent, or the action is destructive; never merge a pull request without user authorization (a plan or spec does not grant merge authority).
...
2. Check reviewer health with `roborev check-agents` when available. Choose a roborev reviewer that is not the implementation model family, and pass it explicitly with `--agent`; do not rely on repo-local `.roborev.toml`, `default_agent`, or daemon defaults because they may point back at the same family.
   - If implementation was Claude/Claude Code, prefer `codex` and run roborev with `--agent codex`. If Codex is unavailable or quota-blocked, try another healthy non-Claude reviewer (`gemini`, `opencode`, `copilot`, `pi`) before skipping. Never use `claude-code` for a Claude implementation.
   - If implementation was Codex, prefer `claude-code`. If Claude is unavailable, try another healthy non-Codex reviewer before skipping. Never use `codex` for a Codex implementation.
   - If implementation was mixed or unknown, use `Preferred roborev reviewer` from `.boil/run.md` only if it is healthy and different from the dominant implementation family; otherwise choose any healthy different reviewer and write the chosen agent back into the iteration summary.
   - **Check the repo config once, not every iteration.** A repo-local `.roborev.toml` with `agent = 'claude-code'` and an empty `review_agent` silently makes EVERY review in that repo run on the implementation family — the hook-fired ones too, which `--agent` on your own invocation cannot reach. Observed live: a Claude-built TUI accumulated ~20 reviews all authored by `claude-code`, so the cross-LLM layer was off for the entire phase while appearing to run. The fix is one line in the repo's `.roborev.toml`: `review_agent = 'codex'`. Verify with `roborev show --job <id> --json | grep agent` after the next review lands.
   - If no different reviewer is available, log "roborev: no different reviewer available, skipped" and file/keep a P2 `tooling` ticket.
